...
package DecNFeEventoProcessor

import Processors.NFeEventoProcessor
import Schemas.NFeEventoSchema
...
object NFeCancelamentoLegadoProcessor {
  // Variáveis externas para o intervalo de meses e ano de processamento
  val ano = 2025
  val mesInicio = 3
  val mesFim = 3
  val tipoDocumento = "nfe_cancelamento"

  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder().appName("ExtractLegadoCancelamentoNFe").enableHiveSupport().getOrCreate()
    import spark.implicits._

    // Obter o esquema da classe CTeOSSchema
    val schema = NFeEventoSchema.createSchema()
    ...
    anoMesList.foreach { anoMes =>
      ...
      val selectedDF = NFeEventoProcessor.generateSelectedDF(parsedDF) // Criando uma nova coluna 'chave_particao' extraindo os dígitos 3 a 6 da coluna 'CHAVE'
      ...
      repartitionedDF
        ...
        .save("/datalake/prata/sources/dbms/dec/nfe/cancelamento")
...
//NFeCancelamentoLegadoProcessor.main(Array())