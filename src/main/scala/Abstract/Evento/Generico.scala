...
import Processors.{BPeEventoProcessor, CTeEventoProcessor, MDFeEventoProcessor, NFeEventoProcessor, NF3eEventoProcessor, NFComEventoProcessor}
import Schemas.{BPeEventoSchema, CTeEventoSchema, MDFeEventoSchema, NF3eEventoSchema, NFComEventoSchema, NFeEventoSchema}
...
object CTeEvento extends DecCancelamentoDiarioProcessor(
  ...
  override def selectColumns(parquetDF: org.apache.spark.sql.DataFrame): org.apache.spark.sql.DataFrame = {
    ...
    parquetDF.select(
      $"XML_DOCUMENTO_CLOB".cast("string").as("xml"),
      $"NSUSVD".cast("string").as("NSUSVD"), // Coluna específica da NFe
      $"DHPROC",
      $"DHEVENTO",
      $"IP_TRANSMISSOR"
    )
