package DecDiarioProcessor.Evento
...
object BPe {
  ...
  val tipoDocumentoEvento = "bpe_evento"
  val prataDocumento = "evento"

  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder().appName("ExtractBPeEvento").enableHiveSupport().getOrCreate()
    ...
    ultimos10Dias.foreach { dia =>
      val parquetPath = s"/datalake/bronze/sources/dbms/dec/processamento/$tipoDocumentoEvento/processar/$dia"
      val parquetPrataPath = s"/datalake/prata/sources/dbms/dec/$tipoDocumento/$prataDocumento"
      val parquetPathProcessado = s"/datalake/bronze/sources/dbms/dec/processamento/$tipoDocumentoEvento/processado/$dia"
