package DecNFCeEventoProcessor

import Processors.NFCeEventoProcessor
import Schemas.NFCeEventoSchema
...
object NFCeCancelamentoDiarioProcessor {
  // Variáveis externas para o tipo de documento
  val tipoDocumento = "nfce"
  val tipoDocumentoCancelamento = "nfce_cancelamento"
  val prataDocumento = "cancelamento"

  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder().appName("ExtractNFCeCancelamento").enableHiveSupport().getOrCreate()
    import spark.implicits._

    // Obter o esquema da classe BPeSchema
    val schema = NFCeEventoSchema.createSchema()
...
//          xmlDF.show()
          ...
          val selectedDF = NFCeEventoProcessor.generateSelectedDF(parsedDF) // Criando uma nova coluna 'chave_particao' extraindo os dígitos 3 a 6 da coluna 'CHAVE'
