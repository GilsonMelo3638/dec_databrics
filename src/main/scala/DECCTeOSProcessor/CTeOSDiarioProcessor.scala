...
object CTeOSDiarioProcessor {
  // Variáveis externas para o tipo de documento
  val tipoDocumento = "cte"
  val prataDocumento = "CTeOS"

  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder().appName("ExtractDiarioCTeOS").enableHiveSupport().getOrCreate()
    ...
    ultimos10Dias.foreach { dia =>
      val parquetPath = s"/datalake/bronze/sources/dbms/dec/processamento/$tipoDocumento/processar_CTeOS/$dia"
      val parquetPrataPath = s"/datalake/prata/sources/dbms/dec/$tipoDocumento/$prataDocumento"
      val parquetPathProcessado = s"/datalake/bronze/sources/dbms/dec/processamento/$tipoDocumento/processar_GVTe/$dia"
