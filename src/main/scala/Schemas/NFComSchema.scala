package Schemas

import org.apache.spark.sql.types._

object NFComSchema {
  def createSchema(): StructType = {
    new StructType()
      .add("NFCom", new StructType()
        .add("infNFCom", new StructType()
          .add("_Id", StringType, true)
          .add("_versao", StringType, true)
          .add("assinante", new StructType()
            .add("NroTermAdic", ArrayType(StringType), true)
            .add("NroTermPrinc", StringType, true)
            .add("cUFAdic", ArrayType(StringType), true)
            .add("cUFPrinc", StringType, true)
            .add("dContratoFim", StringType, true)
            .add("dContratoIni", StringType, true)
            .add("iCodAssinante", StringType, true)
            .add("nContrato", StringType, true)
            .add("tpAssinante", StringType, true)
            .add("tpServUtil", StringType, true))
          .add("gSub", new StructType()
            .add("chNFCom", StringType, true)
            .add("gNF", new StructType()
              .add("CNPJ", StringType, true)
              .add("mod", StringType, true)
              .add("serie", StringType, true)
              .add("nNF", StringType, true)
              .add("CompetEmis", StringType, true)
              .add("hash115", StringType, true)
            )
            .add("motSub", StringType, true)
          )
          .add("gCofat", new StructType()
            .add("chNFComLocal", StringType, true)
          )
          .add("autXML", new StructType()
            .add("CNPJ", StringType, true)
            .add("CPF", StringType, true))
          .add("dest", new StructType()
            .add("xNome", StringType, true)
            .add("CNPJ", StringType, true)
            .add("CPF", StringType, true)
            .add("idOutros", StringType, true)
            .add("indIEDest", StringType, true)
            .add("IE", StringType, true)
            .add("IM", StringType, true)
            .add("enderDest", new StructType()
              .add("CEP", StringType, true)
              .add("UF", StringType, true)
              .add("cMun", StringType, true)
              .add("cPais", StringType, true)
              .add("email", StringType, true)
              .add("fone", StringType, true)
              .add("nro", StringType, true)
              .add("xBairro", StringType, true)
              .add("xCpl", StringType, true)
              .add("xLgr", StringType, true)
              .add("xMun", StringType, true)
              .add("xPais", StringType, true))
)
          .add("det", ArrayType(
            new StructType()
              .add("_chNFComAnt", StringType, true)
              .add("_nItem", StringType, true)
              .add("_nItemAnt", StringType, true)
              .add("prod", new StructType()
                .add("cProd", StringType, true)
                .add("xProd", StringType, true)
                .add("cClass", StringType, true)
                .add("CFOP", StringType, true)
                .add("CNPJLD", StringType, true)
                .add("uMed", StringType, true)
                .add("qFaturada", DoubleType, true)
                .add("vItem", DoubleType, true)
                .add("vDesc", DoubleType, true)
                .add("vOutro", DoubleType, true)
                .add("vProd", DoubleType, true)
                .add("dExpiracao", DoubleType, true)
                .add("indDevolucao", StringType, true))
              .add("imposto", new StructType()
                .add("COFINS", new StructType()
                  .add("CST", StringType, true)
                  .add("vBC", DoubleType, true)
                  .add("pCOFINS", DoubleType, true)
                  .add("vCOFINS", DoubleType, true))
                .add("FUNTTEL", new StructType()
                  .add("vBC", DoubleType, true)
                  .add("pFUNTTEL", DoubleType, true)
                  .add("vFUNTTEL", DoubleType, true))
                .add("FUST", new StructType()
                  .add("vBC", DoubleType, true)
                  .add("pFUST", DoubleType, true)
                  .add("vFUST", DoubleType, true))
                .add("IBSCBS", new StructType() // Campo adicionado
                  .add("CST", StringType, true)
                  .add("cClassTrib", StringType, true)
                  .add("indDoacao", StringType, nullable = true)
                  .add("gIBSCBS", new StructType()
                    .add("gCBS", new StructType()
                      .add("pCBS", DoubleType, true)
                      .add("vCBS", DoubleType, true))
                    .add("gIBSMun", new StructType()
                      .add("pIBSMun", DoubleType, true)
                      .add("vIBSMun", DoubleType, true))
                    .add("gIBSUF", new StructType()
                      .add("gDevTrib", new StructType()
                        .add("vDevTrib", DoubleType, true))
                      .add("pIBSUF", DoubleType, true)
                      .add("vIBSUF", DoubleType, true))
                    .add("vBC", DoubleType, true)
                    .add("vIBS", DoubleType, true))
                )
                .add("ICMS00", new StructType()
                  .add("CST", StringType, true)
                  .add("vBC", DoubleType, true)
                  .add("pICMS", DoubleType, true)
                  .add("vICMS", DoubleType, true)
                  .add("pFCP", DoubleType, true)
                  .add("vFCP", DoubleType, true))
                .add("ICMS20", new StructType()
                  .add("CST", StringType, true)
                  .add("pRedBC", DoubleType, true)
                  .add("vBC", DoubleType, true)
                  .add("pICMS", DoubleType, true)
                  .add("vICMS", DoubleType, true)
                  .add("vICMSDeson", DoubleType, true)
                  .add("cBenef", StringType, true)
                  .add("pFCP", DoubleType, true)
                  .add("vFCP", DoubleType, true))
                .add("ICMS40", new StructType()
                  .add("CST", StringType, true)
                  .add("cBenef", StringType, true)
                  .add("vICMSDeson", DoubleType, true))
                .add("ICMS51", new StructType()
                  .add("CST", StringType, true)
                  .add("vICMSDeson", DoubleType, true)
                  .add("cBenef", StringType, true))
                .add("ICMS60", new StructType()
                  .add("CST", StringType, true)
                  .add("vBCSTRet", DoubleType, true)
                  .add("pST", DoubleType, true)
                  .add("vICMSSTRet", DoubleType, true))
                .add("ICMS90", new StructType()
                  .add("CST", StringType, true)
                  .add("vBC", DoubleType, true)
                  .add("pICMS", DoubleType, true)
                  .add("vICMS", DoubleType, true)
                  .add("vICMSDeson", DoubleType, true)
                  .add("cBenef", DoubleType, true))
                .add("ICMSSN", new StructType()
                  .add("CST", StringType, true)
                  .add("indSN", StringType, true))
                .add("ICMSUFDest", new StructType()
                  .add("cUFDest", StringType, true)
                  .add("vBCUFDest", DoubleType, true)
                  .add("pFCPUFDest", DoubleType, true)
                  .add("pICMSUFDest", DoubleType, true)
                  .add("vFCPUFDest", DoubleType, true)
                  .add("vICMSUFDest", DoubleType, true)
                  .add("vICMSUFEmi", DoubleType, true)
                  .add("cBenefUFDest", StringType, true)
                )
                .add("PIS", new StructType()
                  .add("CST", StringType, true)
                  .add("pPIS", DoubleType, true)
                  .add("vBC", DoubleType, true)
                  .add("vPIS", DoubleType, true))
                .add("indSemCST", StringType, true)
                .add("retTrib", new StructType()
                  .add("vBCIRRF", DoubleType, true)
                  .add("vIRRF", DoubleType, true)
                  .add("vRetCSLL", DoubleType, true)
                  .add("vRetCofins", DoubleType, true)
                  .add("vRetPIS", DoubleType, true)))
              .add("gProcRef", new StructType()
                .add("vItem", DoubleType, true)
                .add("qFaturada", DoubleType, true)
                .add("vProd", DoubleType, true)
                .add("vDesc", DoubleType, true)
                .add("vOutro", DoubleType, true)
                .add("indDevolucao", StringType, true)
                .add("vBC", DoubleType, true)
                .add("pICMS", DoubleType, true)
                .add("vICMS", DoubleType, true)
                .add("vPIS", DoubleType, true)
                .add("vCOFINS", DoubleType, true)
                .add("gProc", new StructType()
                  .add("tpProc", StringType, true)
                  .add("nProcesso", StringType, true)))
              .add("gRessarc", new StructType() // Campo adicionado
                .add("tpRessarc", StringType, true)
                .add("dRef", StringType, true)
                .add("nProtReclama", StringType, true)
                .add("nProcesso", StringType, true) // Campo adicional do schema original
                .add("xObs", StringType, true) // Campo adicional do schema original
              )
              .add("infAdProd", StringType, true)
          ), true)
          .add("emit", new StructType()
            .add("CNPJ", StringType, true)
            .add("IE", StringType, true)
            .add("IEUFDest", StringType, true)
            .add("CRT", StringType, true)
            .add("xNome", StringType, true)
            .add("xFant", StringType, true)
            .add("enderEmit", new StructType()
              .add("CEP", StringType, true)
              .add("UF", StringType, true)
              .add("cMun", StringType, true)
              .add("email", StringType, true)
              .add("fone", StringType, true)
              .add("nro", StringType, true)
              .add("xBairro", StringType, true)
              .add("xCpl", StringType, true)
              .add("xLgr", StringType, true)
              .add("xMun", StringType, true)))
          .add("gFat", new StructType()
            .add("CompetFat", StringType, true)
            .add("codAgencia", StringType, true)
            .add("codBanco", StringType, true)
            .add("codBarras", StringType, true)
            .add("codDebAuto", StringType, true)
            .add("dPerUsoFim", StringType, true)
            .add("dPerUsoIni", StringType, true)
            .add("dVencFat", StringType, true)
            .add("enderCorresp", new StructType()
              .add("CEP", StringType, true)
              .add("UF", StringType, true)
              .add("cMun", StringType, true)
              .add("nro", StringType, true)
              .add("xBairro", StringType, true)
              .add("xCpl", StringType, true)
              .add("xLgr", StringType, true)
              .add("xMun", StringType, true))
            .add("gPIX", new StructType()
              .add("urlQRCodePIX", StringType, true)))
          .add("gFatCentral", new StructType()
            .add("CNPJ", StringType, true)
            .add("cUF", StringType, true))
          .add("gRespTec", new StructType()
            .add("CNPJ", StringType, true)
            .add("email", StringType, true)
            .add("fone", StringType, true)
            .add("xContato", StringType, true))
          .add("ide", new StructType()
            .add("cDV", StringType, true)
            .add("cMunFG", StringType, true)
            .add("cNF", StringType, true)
            .add("cUF", StringType, true)
            .add("dhEmi", StringType, true)
            .add("dhCont", StringType, true)
            .add("finNFCom", StringType, true)
            .add("indCessaoMeiosRede", StringType, true)
            .add("indNotaEntrada", StringType, true)
            .add("indPrePago", StringType, true)
            .add("mod", StringType, true)
            .add("nNF", StringType, true)
            .add("nSiteAutoriz", StringType, true)
            .add("serie", StringType, true)
            .add("tpAmb", StringType, true)
            .add("tpEmis", StringType, true)
            .add("tpFat", StringType, true)
            .add("verProc", StringType, true)
            .add("xJust", StringType, true))
          .add("infAdic", new StructType()
            .add("infAdFisco", StringType, true)
            .add("infCpl", ArrayType(StringType), true))
          .add("total", new StructType()
            .add("IBSCBSTot", new StructType() // Campo adicionado
              .add("gCBS", new StructType()
                .add("vCBS", DoubleType, true)
                .add("vDevTrib", DoubleType, true)
                .add("vDif", DoubleType, true))
              .add("gEstornoCred", new StructType()
                .add("vCBSEstCred", DoubleType, true)
                .add("vIBSEstCred", DoubleType, true))
              .add("gIBS", new StructType()
                .add("gIBSMun", new StructType()
                  .add("vDevTrib", DoubleType, true)
                  .add("vDif", DoubleType, true)
                  .add("vIBSMun", DoubleType, true))
                .add("gIBSUF", new StructType()
                  .add("vDevTrib", DoubleType, true)
                  .add("vDif", DoubleType, true)
                  .add("vIBSUF", DoubleType, true))
                .add("vIBS", DoubleType, true))
              .add("vBCIBSCBS", DoubleType, true))
            .add("ICMSTot", new StructType()
              .add("vBC", DoubleType, true)
              .add("vFCP", DoubleType, true)
              .add("vICMS", DoubleType, true)
              .add("vICMSDeson", DoubleType, true))
            .add("vCOFINS", DoubleType, true)
            .add("vDesc", DoubleType, true)
            .add("vFUNTTEL", DoubleType, true)
            .add("vFUST", DoubleType, true)
            .add("vNF", DoubleType, true)
            .add("vOutro", DoubleType, true)
            .add("vPIS", DoubleType, true)
            .add("vProd", DoubleType, true)
            .add("vRetTribTot", new StructType()
              .add("vIRRF", DoubleType, true)
              .add("vRetCSLL", DoubleType, true)
              .add("vRetCofins", DoubleType, true)
              .add("vRetPIS", DoubleType, true))
            .add("vTotDFe", DoubleType, true)))
        .add("infNFComSupl", new StructType()
          .add("qrCodNFCom", StringType, true)))
      .add("_dhConexao", StringType, true)
      .add("_ipTransmissor", StringType, true)
      .add("_nPortaCon", StringType, true)
      .add("_versao", StringType, true)
      .add("_xmlns", StringType, true)
      .add("protNFCom", new StructType()
        .add("_versao", StringType, true)
        .add("infProt", new StructType()
          .add("_Id", StringType, true)
          .add("cStat", StringType, true)
          .add("chNFCom", StringType, true)
          .add("dhRecbto", StringType, true)
          .add("digVal", StringType, true)
          .add("nProt", StringType, true)
          .add("tpAmb", StringType, true)
          .add("verAplic", StringType, true)
          .add("xMotivo", StringType, true)))
  }
}