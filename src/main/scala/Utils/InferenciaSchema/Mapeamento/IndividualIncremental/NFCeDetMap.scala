  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val camposNaoMapeados = schemaInferido
        .diff(schemaOficial)
        .filter { campo =>
          val lower = campo.toLowerCase
          val partes = lower.split("\\.")

          partes.contains("det") &&                     // ✔ só det
            !partes.contains("signature") &&              // ❌ remove signature
            !partes.contains("infnfe")                    // ❌ remove infNFe
        }
