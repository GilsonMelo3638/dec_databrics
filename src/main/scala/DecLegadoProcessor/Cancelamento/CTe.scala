...
import Processors.CTeEventoProcessor
import Schemas.CTeEventoSchema
...
object CTe {
  ...
  val mesInicio = 3
  val mesFim = 3
  val tipoDocumento = "cte_cancelamento"

  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder().appName("ExtractLegadoCancelamentoCTe").enableHiveSupport().getOrCreate()
    import spark.implicits._

    // Obter o esquema da classe CTeOSSchema
    val schema = CTeEventoSchema.createSchema()
    ...
    anoMesList.foreach { anoMes =>
      val parquetPath = s"/datalake/bronze/sources/dbms/dec/processamento/cte_cancelamento/processar/20260209"
      ...
      val selectedDF = CTeEventoProcessor.generateSelectedDF(parsedDF) // Criando uma nova coluna 'chave_particao' extraindo os dígitos 3 a 6 da coluna 'CHAVE'
      ...
      repartitionedDF
        ...
        .save("/datalake/prata/sources/dbms/dec/cte/cancelamento")
