...
object GVTeDiarioProcessor {
  // Variáveis externas para o tipo de documento
  val tipoDocumento = "cte"
  val prataDocumento = "GVTe"

  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder().appName("ExtractDiarioGVTe").enableHiveSupport().getOrCreate()
    ...
    ultimos10Dias.foreach { dia =>
      val parquetPath = s"/datalake/bronze/sources/dbms/dec/processamento/$tipoDocumento/processar_GVTe/$dia"
