...
object SqoopNF3eProcessor {
  ...
  """
    ...
    if (minMaxDF.isEmpty) {
      println("Nenhum dado encontrado para o período especificado. Nada a fazer.")
      spark.stop()
      System.exit(0)
    }
    ...
    // Verifica se o intervalo de valores é válido
    if (min == max) {
      println(s"A coluna '$splitByColumn' possui apenas um valor único ($min). Usando uma única partição.")
      ...
      val targetDir = s"/datalake/bronze/sources/dbms/dec/processamento/nf3e/processar/$anoMesDia"
