...
object diarioGenericoEvento {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val configs = List(
      ...
      ("procEventoCTe", "cte", "CTE_SVD", "cte_evento", "NSUSVD", 5),  // NFe com 5 partições
      ("procEventoBPe", "bpe", "BPE", "bpe_evento", "NSU", 1) , // MDFe com 1 partição
        ...
        if (fs.exists(new Path(targetDirProcessar)) || fs.exists(new Path(targetDirProcessado))) {
          ...
          val baseQuery =
            ...
            SELECT
              ...
              f.SEQ_EVENTO,
              f.TP_EVENTO
            FROM DEC_DFE_${tabela}_EVENTO f
...
//diarioGenericoEvento.main(Array())