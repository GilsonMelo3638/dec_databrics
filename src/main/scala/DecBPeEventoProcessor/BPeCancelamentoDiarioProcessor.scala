...
object BPeCancelamentoDiarioProcessor {
  // Variáveis externas para o tipo de documento
  val tipoDocumento = "bpe"
  val tipoDocumentoCancelamento = "bpe_cancelamento"
  val prataDocumento = "cancelamento"

  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder().appName("ExtractBPeCancelamento").enableHiveSupport().getOrCreate()
